...
module.exports.create = function(req, res){
    ...
    newProduct.save(function(err){
        if(err){
            ...
            res.send(`Yup! Product is added with name ${req.body.name}`);
        }
    });
}
    ...
    Product.deleteOne(
        ...
        function(err){
            if(err){
                ...
                res.send({
                    message: `Your product is deleted  successfully with id ${req.params.productID}`
                });
            }
        });
}
            ...
            Product.findByIdAndUpdate(ID, {quantity: newQty}, function(err, updatedProduct){
                if(err){
                    ...
                    res.send({
                        ...
                        message: `Yeah! product is update with quantity ${newQty}`
                    });
                }
            });
        }
